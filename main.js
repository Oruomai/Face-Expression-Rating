...
class FaceRating {
    ...
    async update(e) {
        ...
        const item = await fetch("https://rating-expression.herokuapp.com/")
        const number = await item.json()
        
        const percent = number[0].Rating;

        document.getElementById("log").innerHTML=percent;
